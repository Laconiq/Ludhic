...
export default function GenreBadge({ genre, variant, showHash = true, href }: GenreBadgeProps) {
  if (variant === 'card') {
    return (
      <span class="bg-[var(--bg-tertiary)] border border-[var(--border-primary)] text-[var(--text-primary)] font-gaming font-medium text-[0.7rem] uppercase tracking-[0.05em] px-3 py-1 rounded-full">
        {showHash ? '#' : ''}{genre}
      </span>
    );
  }
  ...
  return <span class={className}>{genre}</span>;
}
